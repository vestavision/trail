...
func TestNormalizePromotesPortableFields(t *testing.T) {
	...
	envelope := wire.Envelope{
		...
		Events: []wire.Event{{
			...
			FlowID: flowID, ExecutionID: executionID, ScopeType: "tenant", ScopeID: "tenant_1",
			Fields: []wire.Field{
				...
				{Key: "provider", Type: "string", Text: "bank"},
				{Key: "error", Type: "error", Text: "declined"},
	...
	if record.FlowStatus != "awaiting_review" || record.ExecutionStatus != "failed" || record.RetentionClass != "audit" || record.Scope.Type != "tenant" || record.Scope.ID != "tenant_1" || record.Provider != "bank" || !record.HasError || record.HTTP.StatusCode != 503 || record.HTTP.Success {
