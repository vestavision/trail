...
var insertEventSQL = `INSERT INTO trail_events (` +
	...
	`) VALUES (` + placeholders(43) + `) ON CONFLICT (event_id) DO NOTHING`

func eventValues(e storage.EventRecord, fields, payloads []byte) []any {
	return []any{e.Timestamp, idBytes(e.ID), idBytes(e.BatchID), e.Delivery.Stream, e.Delivery.StreamSequence, e.Delivery.Consumer, e.Delivery.ConsumerSequence, e.Delivery.Delivered, e.Delivery.ReceivedAt, e.Metadata.Service, e.Metadata.Environment, e.Metadata.Version, e.Kind, e.Level.String(), nullableID(e.FlowID), nullableID(e.ExecutionID), nullableID(e.ParentExecutionID), nullableID(e.RetryOfExecutionID), e.ExecutionAttempt, string(e.ExecutionSource), e.Scope.Type, e.Scope.ID, e.EntityType, e.EntityID, nullableID(e.ParentID), e.FlowStatus, e.ExecutionStatus, e.ExecutionKind, e.Provider, e.HasError, fields, e.HTTP.Method, e.HTTP.Scheme, e.HTTP.Host, e.HTTP.Path, e.HTTP.StatusCode, e.HTTP.Duration.Nanoseconds(), e.HTTP.Success, e.HTTP.RequestSize, e.HTTP.ResponseSize, e.HTTP.RequestPreview, e.HTTP.ResponsePreview, payloads}
}
