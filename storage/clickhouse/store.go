...
const insertEvents = `INSERT INTO trail_events (
    ...
    entity_type, entity_id, parent_event_id, flow_status, execution_status, execution_kind,
...
func appendEvent(batch driver.Batch, event storage.EventRecord) error {
	...
	return batch.Append(
		...
		event.ExecutionAttempt, string(event.ExecutionSource), event.EntityType, event.EntityID, idBytes(event.ParentID),
		event.FlowStatus, event.ExecutionStatus, event.ExecutionKind,
